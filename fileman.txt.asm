...
diskEmptyString4: db "                             PRESS [ESCAPE] TO EXIT                             ", 0
...
previousPageInstructionString:	db " [LEFT]: previous page", 0
nextPageInstructionString:		db "[RIGHT]: next page", 0
cursorUpInstructionString:		db "   [UP]: cursor up", 0
cursorDownInstructionString:	db " [DOWN]: cursor down", 0
exitInstructionString:			db "  [ESC]: exit", 0

; disk status box variables
DISK_STATUS_BOX_CONTENTS_HEIGHT equ 2
...
; Loads current file into memory, in preparation for a file copy
;
handle_read_file:
	pusha
	pushf
...
print_disk_empty_print:
	...
	cmp byte [loadedFileExists], 0
	je print_disk_empty_last_line	; if no file is stored, we skip over 
									; the third line (the one about writing
									; the stored file)
	mov si, diskEmptyString3
	int 97h						; print
print_disk_empty_last_line:
...
draw_instructions_box:
	...
	int 9Eh					; move cursor
	mov si, previousPageInstructionString
	int 97h					; print
	
	mov bh, INSTRUCTIONS_BOX_TOP + 7
	int 9Eh					; move cursor
	mov si, nextPageInstructionString
	int 97h					; print
	
	mov bh, INSTRUCTIONS_BOX_TOP + 8
	int 9Eh					; move cursor
	mov si, cursorUpInstructionString
	int 97h					; print
	
	mov bh, INSTRUCTIONS_BOX_TOP + 9
	int 9Eh					; move cursor
	mov si, cursorDownInstructionString
	int 97h					; print
...
initialize_disk_empty_wait_for_key:
	...
	cmp ah, COMMON_SCAN_CODE_F5
	je initialize				; if user presses "re-scan", then re-initialize
	cmp ah, COMMON_SCAN_CODE_ESCAPE
	...
	cmp ax, 0						; did we actually write the file?
	je initialize					; yes, so re-initialize
									; no, so read next key
...
initialize_exit_program:
	mov bx, word [allocatedSegment]
	int 92h							; deallocate memory
	int 95h							; user chose to exit program
